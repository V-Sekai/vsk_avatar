...
static func get_digit_chain(p_skeleton: Skeleton3D, p_side: int, p_digit: int) -> PackedInt32Array:
	return get_chain(p_skeleton, p_side,\
	"%s%s" % [
		avatar_constants_const.get_name_for_digit(p_digit),
		avatar_constants_const.get_name_for_digit_joint(avatar_constants_const.DIGIT_JOINT_PROXIMAL if p_digit == avatar_constants_const.DIGIT_THUMB else avatar_constants_const.DIGIT_JOINT_METACARPAL)
	],
	"%s%s" % [
		avatar_constants_const.get_name_for_digit(p_digit),
