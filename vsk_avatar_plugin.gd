...
func update_menu_options() -> void:
	if option_button:
		...
		(
			option_button
			. get_popup()
			. add_item("Upload Avatar", avatar_definition_editor_const.MENU_OPTION_UPLOAD_AVATAR)
		)
		(
			option_button
			. get_popup()
...
func _enter_tree() -> void:
	...
	var clear_icon: Texture = (
		editor_interface . get_base_control() . get_theme_icon("Clear", "EditorIcons")
	)
	var bone_icon: Texture = (
		editor_interface . get_base_control() . get_theme_icon("BoneAttachment", "EditorIcons")
	)
