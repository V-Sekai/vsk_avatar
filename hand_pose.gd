@tool
extends Resource

@export var thumb_proximal: Transform3D = Transform3D()
@export var thumb_intermediate: Transform3D = Transform3D()
@export var thumb_distal: Transform3D = Transform3D()

@export var index_proximal: Transform3D = Transform3D()
@export var index_intermediate: Transform3D = Transform3D()
@export var index_distal: Transform3D = Transform3D()

@export var middle_proximal: Transform3D = Transform3D()
@export var middle_intermediate: Transform3D = Transform3D()
@export var middle_distal: Transform3D = Transform3D()

@export var ring_proximal: Transform3D = Transform3D()
@export var ring_intermediate: Transform3D = Transform3D()
@export var ring_distal: Transform3D = Transform3D()
 
@export var little_proximal: Transform3D = Transform3D()
@export var little_intermediate: Transform3D = Transform3D()
@export var little_distal: Transform3D = Transform3D()

