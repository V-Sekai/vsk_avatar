...
const IK_POINT_LEFT_HAND_BASIS_GLOBAL = Basis(Vector3(-1, 0, 0), Vector3(0, 0.707, -0.707), Vector3(0.0, -0.707, -0.707))
const IK_POINT_RIGHT_HAND_BASIS_GLOBAL = Basis(Vector3(-1, 0, 0), Vector3(0, 0.707, -0.707), Vector3(0.0, -0.707, -0.707))
...
func update_local_transforms() -> void:
	...
	if tracker_collection_input:
		...
		if tracker_collection_input.left_hand_spatial:
			...
			if controller:
				tracker_collection_input.left_hand_spatial.transform = Transform3D(controller.transform.basis, (controller.transform.origin  + origin_offset - camera_offset)) \
		...
		if tracker_collection_input.right_hand_spatial:
			...
			if controller:
				tracker_collection_input.right_hand_spatial.transform = Transform3D(controller.transform.basis, (controller.transform.origin + origin_offset - camera_offset)) \
					.translated(Vector3(IK_HAND_OFFSET.x, IK_HAND_OFFSET.y, IK_HAND_OFFSET.z)) \
