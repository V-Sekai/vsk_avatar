...
static func fix_bone(p_root : Spatial, p_skeleton : Skeleton, p_id : int, p_global_basis : Basis, p_transform_offsets : Array) -> Array:
	...
	var global_pose : Transform = skeleton_global_transform * p_skeleton.get_bone_global_pose(p_id)
			
	var custom_transform : Transform = p_skeleton.get_bone_custom_pose(p_id)
	var difference : Quat = p_global_basis.inverse().get_rotation_quat() * global_pose.basis.get_rotation_quat()
	p_skeleton.set_bone_custom_pose(p_id, custom_transform * Transform(difference).inverse())
	p_transform_offsets[p_id] *= Transform(difference).inverse()
		
	#
	for child_id in get_bone_children(p_skeleton, p_id):
		var child_custom_transform : Transform = p_skeleton.get_bone_custom_pose(child_id)
		var rest_transform : Transform = p_skeleton.get_bone_rest(child_id)
		...
		p_skeleton.set_bone_custom_pose(child_id, child_custom_transform * child_transform_offset)
		
		p_transform_offsets[child_id] *= child_transform_offset
		
	return p_transform_offsets

static func fix_chain(p_root : Spatial, p_skeleton : Skeleton, p_start_id : int, p_end_id : int, p_valid_bone_ids : PoolIntArray, p_global_basis : Basis, p_transform_offsets : Array) -> Array:
	...
	for id in bone_chain:
		if array_util_const.pool_int_array_find(p_valid_bone_ids, id) != -1:
			p_transform_offsets = fix_bone(p_root, p_skeleton, id, p_global_basis, p_transform_offsets)
		
	return p_transform_offsets
	
static func fix_arm(p_root : Spatial, p_humanoid_data : humanoid_data_const, p_skeleton : Skeleton, p_transform_offsets : Array, p_side : int) -> Array:
	...
	p_transform_offsets = fix_chain(p_root, p_skeleton, arm_start_id, arm_end_id, valid_bone_ids, global_basis, p_transform_offsets)
	
	return p_transform_offsets
	
	
static func fix_digit(p_root : Spatial, p_humanoid_data : humanoid_data_const, p_skeleton : Skeleton, p_transform_offsets : Array, p_side : int, p_digit_id : int, p_global_basis : Basis) -> Array:
	...
	p_transform_offsets = fix_chain(p_root, p_skeleton, start_id, end_id, valid_bone_ids, p_global_basis, p_transform_offsets)
	
	print("Digit Fixed!")
	
	return p_transform_offsets
	
static func fix_digits(p_root : Spatial, p_humanoid_data : humanoid_data_const, p_skeleton : Skeleton, p_transform_offsets : Array, p_side : int) -> Array:
	var direction_name : String = avatar_constants_const.get_name_for_side(p_side)
	var global_basis : Basis = LEFT_ARM_BASIS_GLOBAL if p_side == avatar_constants_const.SIDE_LEFT else RIGHT_ARM_BASIS_GLOBAL

	p_transform_offsets = fix_digit(p_root, p_humanoid_data, p_skeleton, p_transform_offsets, p_side, avatar_constants_const.DIGIT_THUMB, global_basis.rotated(Vector3(1.0, 0.0, 0.0), PI * 0.5))
	for i in range(avatar_constants_const.DIGIT_INDEX, avatar_constants_const.DIGIT_LITTLE):
		p_transform_offsets = fix_digit(p_root, p_humanoid_data, p_skeleton, p_transform_offsets, p_side, i, global_basis)
	
	return p_transform_offsets
	
static func fix_spine(p_root : Spatial, p_humanoid_data : humanoid_data_const, p_skeleton : Skeleton, p_transform_offsets : Array) -> Array:
	...
	p_transform_offsets = fix_chain(p_root, p_skeleton, spine_start_id, spine_end_id, valid_bone_ids, HIPS_BASIS_GLOBAL, p_transform_offsets)
	
	return p_transform_offsets
	
static func fix_neck(p_root : Spatial, p_humanoid_data : humanoid_data_const, p_skeleton : Skeleton, p_transform_offsets : Array) -> Array:
	...
	p_transform_offsets = fix_chain(p_root, p_skeleton, neck_start_id, neck_end_id, valid_bone_ids, global_basis, p_transform_offsets)
	
	return p_transform_offsets
	
static func fix_leg(p_root : Spatial, p_humanoid_data : humanoid_data_const, p_skeleton : Skeleton, p_transform_offsets : Array, p_side : int) -> Array:
	var direction_name : String = avatar_constants_const.get_name_for_side(p_side)
	...
	p_transform_offsets = fix_chain(p_root, p_skeleton, leg_start_id, leg_end_id, valid_bone_ids, global_basis, p_transform_offsets)
	
	return p_transform_offsets
	
static func fix_foot(p_root: Spatial, p_humanoid_data : humanoid_data_const, p_skeleton : Skeleton, p_transform_offsets : Array, p_side : int) -> Array:
	var direction_name : String = avatar_constants_const.get_name_for_side(p_side)
	...
	p_transform_offsets = fix_bone(p_root, p_skeleton, foot_id, global_basis, p_transform_offsets)
	
	return p_transform_offsets
	
static func fix_eye(p_root : Spatial, p_humanoid_data : humanoid_data_const, p_skeleton : Skeleton, p_transform_offsets : Array, p_side : int) -> Array:
	var direction_name : String = avatar_constants_const.get_name_for_side(p_side)
	var global_basis : Basis = HEAD_BASIS_GLOBAL
	...
	p_transform_offsets = fix_bone(p_root, p_skeleton, eye_id, global_basis, p_transform_offsets)
	
	return p_transform_offsets

static func reset_skeleton_and_bind_pose(p_skeleton, p_skins : Array, p_default_skins : Array) -> Array:
	for i in range(0, p_skeleton.get_bone_count()):
		p_skeleton.set_bone_custom_pose(i, Transform())
...
static func clear_rotations(p_skeleton : Skeleton, p_humanoid_data : humanoid_data_const, p_skins : Array, p_default_skins : Array) -> Array:
	p_skins = reset_skeleton_and_bind_pose(p_skeleton, p_skins, p_default_skins)
		
	return p_skins

static func fix_rotations_internal(p_root : Spatial, p_skeleton : Skeleton, p_humanoid_data : humanoid_data_const, p_skins : Array, p_default_skins : Array) -> Array:
	var rest_transform_local_offsets : Array = []
	...
	for i in range(0, p_skeleton.get_bone_count()):
		...
		t_pose_transform_local_offsets.push_back(p_skeleton.get_bone_custom_pose(i))
		rotation_fix_transform_local_offsets.push_back(Transform())
	
	rotation_fix_transform_local_offsets = fix_spine(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets)
	rotation_fix_transform_local_offsets = fix_neck(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets)
	rotation_fix_transform_local_offsets = fix_eye(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, false)
	rotation_fix_transform_local_offsets = fix_eye(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, true)
	rotation_fix_transform_local_offsets = fix_arm(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, false)
	rotation_fix_transform_local_offsets = fix_arm(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, true)
	rotation_fix_transform_local_offsets = fix_leg(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, false)
	rotation_fix_transform_local_offsets = fix_leg(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, true)
	rotation_fix_transform_local_offsets = fix_foot(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, false)
	rotation_fix_transform_local_offsets = fix_foot(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, true)
	rotation_fix_transform_local_offsets = fix_digits(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, avatar_constants_const.SIDE_LEFT)
	rotation_fix_transform_local_offsets = fix_digits(p_root, p_humanoid_data, p_skeleton, rotation_fix_transform_local_offsets, avatar_constants_const.SIDE_RIGHT)
	...
	if APPLY_AS_REST:
		...
		for i in range(0, p_skeleton.get_bone_count()):
			p_skeleton.set_bone_custom_pose(i, t_pose_transform_local_offsets[i] * rotation_fix_transform_local_offsets[i])
...
static func fix_rotations(p_root : Spatial, p_skeleton : Skeleton, p_humanoid_data : humanoid_data_const) -> int:
	...
	skins = fix_rotations_internal(p_root, p_root._skeleton_node, p_root.humanoid_data, skins, [])
